...
def test_check_input():
    ...
    with pytest.raises(ValueError):
        ...
        bad_olp_ab_ab *= np.diag(bad_olp_ab_ab) ** (-1)
        _check_input(olp_ab_ab=bad_olp_ab_ab)
    with pytest.raises(ValueError):
        bad_olp_ab_ab = np.random.rand(10, 10)
        bad_olp_ab_ab *= np.diag(bad_olp_ab_ab) ** (-1)
        _check_input(olp_ab_ab=(bad_olp_ab_ab + bad_olp_ab_ab.T) / 2)
    ...
    with pytest.raises(ValueError):
        ...
        bad_olp_aao_aao *= np.diag(bad_olp_aao_aao) ** (-1)
        _check_input(olp_aao_aao=bad_olp_aao_aao)
    with pytest.raises(ValueError):
        bad_olp_aao_aao = np.random.rand(5, 5)
        bad_olp_aao_aao *= np.diag(bad_olp_aao_aao) ** (-1)
        _check_input(olp_aao_aao=(bad_olp_aao_aao + bad_olp_aao_aao.T) / 2)
